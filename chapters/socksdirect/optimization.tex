...
共享内存的过程类似，因为共享内存和RDMA都支持写原语。

要判断环形缓冲区是否已满，发送方将保持\textit {队列信用} 计数，指示环形缓冲区中的空闲字节数。
当发件人将数据包入队时，它会消耗信用。 当接收器使消息出列时，它会在本地递增计数器，并在计数器超过环形缓冲区大小的一半时在发送者的内存中写入\textit {信用返回标志}。 发送者在检测到标志时重新获得队列信用。
...
对于共享内存，只有一个由两个进程共享的环形缓冲区拷贝，并且同步由高速缓存一致性硬件完成。
...
其次，每个队列可以在\textit {轮询}和\textit {中断}模式之间切换。监视器的队列始终处于轮询模式。每个队列的接收者维护一个连续空轮询的计数器。当它超过阈值时，接收器向发送器发送消息，通知队列正在进入中断模式，并在短时间后停止轮询。当发送方以中断模式写入队列时，它还会通知监视器，监视器将通知接收方恢复轮询。
