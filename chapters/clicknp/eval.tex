...
HTTPS提供三件事来确保安全性。首先，它在连接设置期间验证Web服务器的实体。其次，它加密用户和Web服务器之间的数据传输。第三，它检查数据的完整性。在这三种机制中，第一认证部分是计算密集程度最高的部分。
...
Throughput (10,000 samples in a batch): 17.5 us per sample

Throughput bottleneck is DenseLayers
Latency (1 sample in a batch): 29.6 us
PCIe I/O: 4 us (PCIe I/O has much lower latency than OpenCL API (1.8 ms))
Sparse layers: 5 us
Rowstack: 3 us
Dense layers: 17.5 us

Latency breakdown of the 17.5 us version (170.9 MHz):
1.	Computation. 640*128, 128*640, 640*64, 64*640 takes 5*128 + 1*640 + 5*64 + 1*640 = 2240 cycles (13.1 us).


2.	Communication. This is overlapped with computation.
3.	Denselayers pipeline latency. 144 cycles per layer, total 576 cycles (3.4 us).


The 4 layers have 640*128 + 128*640 + 640*64 + 64*640 = 245760 float multiplications. 

The latency breakdown is:
1.	Computation. 640*128, 128*640, 640*64, 64*640 takes 1280 + 1280 + 640 + 640 = 3840 cycles (this 18.8 us is base latency)
2.	Communication. Input, output and channel transfer takes 640 + 128 + 640 + 64 + 640 = 2112 cycles
3.	Dense\_layers pipeline latency. 139 cycles per layer, total 556 cycles.
The total latency is 6508 cycles (31.9 us), slightly higher than 30.9 us measured latency. This is because computation, communication and pipeline latency are overlapped in the pipeline.
