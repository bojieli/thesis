...
%This paper presents KV-Direct, a high performance key-value store with programmable network interface card (网卡) which is deployed at scale in datacenters.
%KV-Direct extends RDMA primitives from memory operations to key-value operations, moves 键值 processing to the programmable 网卡 on server, thus enabling remote direct key-value access.
...
%KV-Direct also leverages the DRAM on the 网卡 for caching and load balancing.
%A programmable 网卡 with KV-Direct can process up to 160M key-value operations per second, whose throughput is equivalent to 32 CPU cores, power efficiency is 16x higher than CPU, and tail latency is 10x lower than CPU.
