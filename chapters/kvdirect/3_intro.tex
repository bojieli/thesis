...
本章提出\oursys{}，一个新的内存中键值系统，利用数据中心的可编程网卡。
...
本章的其余部分安排如下。 \S \ref {kvdirect:sec:background}显示了背景，阐明了设计目标和挑战。 \S \ref {kvdirect:sec:architecture}描述了\oursys{} 的设计。 \S \ref {kvdirect:sec:implementation}显示了\oursys{} 的实现细节。 \S \ref {kvdirect:sec:evaluation}是对\oursys{} 的评估。 \S \ref {kvdirect:sec:extensions}讨论了进一步的扩展。\S \ref {kvdirect:sec:related} 将讨论相关工作，并在\S \ref {kvdirect:sec:conclusion}中得出结论。
